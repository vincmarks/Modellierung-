...
p1 = scatter(10.0 .^(-1:-1:-10) , err, title=L"\vert f'(x_1) - (f_2-f_0)/(2h) \vert \quad vs. h ", yaxis=:log, xaxis=:log, label=L"\vert f'(x_1) - (f_2-f_0)/(2h) \vert", xlabel=L"10^{-i}", ylabel="Error", legend =:bottomright)
vline!(p1, [sqrt(eps())], label=L"h = \sqrt{\epsilon}", color=:red, linestyle=:dash)
savefig(p1, joinpath(plot_folder, "error_sin.pdf"))
...
# sqrt(eps()) ≈ 1.4901161193847656e-8. actually we can see this in the plot itself. For h < sqrt(eps()) the error starts to increase again. This is due to rounding errors in numerical differentiation.
...
savefig(p2, joinpath(plot_folder, "error_sin_integration.pdf"))
...
savefig(p3, joinpath(plot_folder, "error_exp_integration.pdf"))
